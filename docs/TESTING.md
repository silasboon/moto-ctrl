...
| Starter request while "engine running" → rejected | Automated + GUI | `mc_output`'s starter protection is real. `engine_running` has two independent contributors: opt-in voltage-based detection (**off by default** — a booster pack/bench PSU can't be told apart from a real alternator, so this is never assumed) derived from calibrated battery voltage every tick when enabled, and an unconditional override that forces it false whenever an assigned ignition channel is off, regardless of that toggle. Either drives both the starter guard and `mc_lock`'s parked-detection guard. `test_diag.c` covers the toggle and the override in isolation, including that disabling the toggle mid-session clears an already-true reading rather than leaving it stale; `SET_ENGINE_RUNNING` nudges the injected battery voltage and force-enables the toggle so it stays a one-call convenience for the GUI (§3's diagnostics note), but does not bypass the ignition-off override. The lock system adds the analogous immobilize check (starter *and* ignition refused while LOCKED, any source) on top |
